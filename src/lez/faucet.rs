...
//!
//! This is the single canonical implementation: `lez-mcp` re-exports from
//! here rather than keeping its own copy (see `lez-mcp/src/faucet.rs`).
...
/// caller drops the request), this guard's `Drop` fires and flips the flag, so
/// the detached blocking solver stops promptly at its next check instead of
/// hashing on. When the solve completes normally the guard is
/// [`disarm`](Self::disarm)ed and then dropped *normally* — no `mem::forget`,
/// so its `Arc` clone is released rather than leaked once per successful solve.
struct CancelOnDrop {
    flag: Arc<AtomicBool>,
    armed: bool,
}
...
///   prevents unbounded concurrent solves under cancellation.

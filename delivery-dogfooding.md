...
- Delivery offer discovery used the public Delivery preset (`mode: Core`, `preset: logos.dev`) with per-instance `portsShift`; no embedded Waku rendezvous node or `waku-bindings` path was used. (Historical: this run predates the migration to the `logos.test` preset. The app now defaults to `logos.test`, the stability-guaranteed fleet upstream recommends — see `docs/local-dev-harness.md` and logos-co/logos-delivery-module#84. `logos.dev` was dropped because it "is subtle to change at any moment": an unannounced re-genesis moved it to Waku cluster 3 and forced a hand-maintained cluster override that logos.test makes unnecessary.)
